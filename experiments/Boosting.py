...
class BoostingExperiment(experiments.BaseExperiment):
    ...
    def perform(self):
        ...
        #sys.exit(1)
        if self._details.ds_name == "QSARbiodegradationData":
            best_params = {'base_estimator__max_depth': 5, 'learning_rate': 1, 'n_estimators': 20, 'base_estimator__class_weight':'balanced',
             'base_estimator__criterion':'entropy','base_estimator__min_impurity_decrease':0, 'base_estimator__min_samples_leaf':1,'base_estimator__min_samples_split':2,
             'base_estimator__min_weight_fraction_leaf':0, 'base_estimator__presort':'FALSE', 'base_estimator__splitter': 'best', 'base_estimator__random_state':1234 }
        #
        # Dataset 2:
        if self._details.ds_name == "VoiceData":
            best_params = {'base_estimator__max_depth': 4, 'learning_rate': 1, 'n_estimators': 100, 'base_estimator__class_weight':'balanced',
             'base_estimator__criterion':'entropy','base_estimator__min_impurity_decrease':0, 'base_estimator__min_samples_leaf':1,'base_estimator__min_samples_split':2,
             'base_estimator__min_weight_fraction_leaf':0, 'base_estimator__presort':'FALSE', 'base_estimator__splitter': 'best', 'base_estimator__random_state':1234 }
        ...
        #                               iteration_details=iteration_details,best_params=best_params, threads=self._details.threads,
        #                               verbose=self._verbose, iteration_lc_only=True)
        experiments.perform_experiment(self._details.ds, self._details.ds_name, self._details.ds_readable_name,
                                       of_booster, 'Boost_OF', 'Boost', of_params, seed=self._details.seed,
                                       iteration_details=iteration_details, threads=self._details.threads,
                                       verbose=self._verbose, iteration_lc_only=True)
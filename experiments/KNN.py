...
class KNNExperiment(experiments.BaseExperiment):
    ...
    def perform(self):
        ...
        if self._details.ds_name == "QSARbiodegradationData":
            best_params = {'metric': 'manhattan','leaf_size':30, 'n_jobs':-1 ,'n_neighbors': 13, 'p':2, 'weights': 'uniform'}
        #
        # Dataset 1:
        if self._details.ds_name == "VoiceData":
            best_params = {'metric': 'manhattan','leaf_size':30, 'n_jobs':-1 ,'n_neighbors': 1, 'p':2,  'weights': 'uniform'}
